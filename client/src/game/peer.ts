// WebRTC transporta la partida. HTTP sólo intercambia oferta/respuesta al entrar.
export class Peer {
  ...
  async connect() {
    try {
      if (!globalThis.RTCPeerConnection) throw new Error("Este navegador no admite WebRTC. Usá Chrome, Edge o Firefox actualizado.");
      ...
      pc.onconnectionstatechange = () => {
        if (pc.connectionState === "failed") this.fail("No se pudo conectar entre las dos casas. La red puede requerir un servidor TURN.");
      ...
      while (!this.closed && !pc.remoteDescription) {
        const { description } = await this.api("signal");
        if (description) {
      ...
      const connectionDeadline = performance.now() + 35_000;
      while (!this.ready && !this.closed && performance.now() < connectionDeadline) await new Promise(resolve => setTimeout(resolve, 250));
      if (!this.ready && !this.closed) throw new Error("Las redes no permitieron la conexión directa. Hace falta configurar TURN para estas conexiones.");
    } catch (error) {
      if (!this.closed) this.fail(error instanceof Error ? error.message : "No se pudo conectar.");
    }
  }
  ...
  private attach(channel: RTCDataChannel) {
    ...
    channel.onclose = () => { if (!this.closed) this.fail("La partida se cerró. Volvé para crear otra."); };
